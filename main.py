...
concat = [None,None]
concat[0] = Concat( 0, 490)
concat[1] = Concat(70, 540)
...
for i in range(3):
	boxes[0][1][i] = (Hotvec(x-15+225*i,y+50))

for i in range(3):
	boxes[0][2][i] = (W(x+225*i,y+140))

for i in range(3):
	boxes[0][3][i] = (LSTM(x+225*i,y+230))

for i in range(3):
	boxes[0][4][i] = (LSTM(x+225*i,y+320))

for i in range(3):
	boxes[0][5][i] = (Emb(x+225*i,y+410))

wave = [[None for j in range(2)] for i in range(3)]
for i in range(2):
	box = boxes[0][3+i][2]
	wave[0][i] = Wave(box.r+40,box.y)
...
for i in range(1,3):
	...
	for j in range(3):
		boxes[i][4][j] = (Word(x-15+225*j,y+320))

	for j in range(2):
		box = boxes[i][j][2]
		wave[i][j] = Wave(box.r+40,box.y)
